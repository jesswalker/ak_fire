...
# portions.  A id is added to each polygon created from the union to identify
...
#
# Note that the file produced has 3-4 entries for each union:
# one for each separate polygon created. Requires post-processing in R to
# calculate the fire interval. See process_intersected_fire_pairs.R
...
filename_add = "_junk2"  # tag for filename
out_shp_name = "union_test_" + str(reburn_num) + "_" + filename_add + ".shp"
# ********************************************************
    ...
    try:
        if row.getValue(burn_num) == reburn_num:
            ...
            for row2 in cursor2:
                ...
                #arcpy.SelectLayerByAttribute_management("orig_polys_lyr", "NEW_SELECTION", sql_both_polys)
            ...
            # Intersect current and previous burns
                ...
                #arcpy.Intersect_analysis("in_memory/orig_fc", "in_memory/reburn_poly")
