...
Given the wide range of FRP values, log(FRP) transformations are appropriate.
...
```
The above plot is unhelpful because FRP is evaluated across all land cover types.  Below is the same data broken out by land cover, where "land cover" is assigned to whichever 2001 EVT class covered the greatest proportion of each 1-km MODIS pixel.
```{r, echo=FALSE, eval=TRUE}
...
```
Black Spruce, Burned, Tundra, and Wetland appear to have possibly decreasing FRP relationships with burn number. White Spruce may have an increasing trend. ANOVA results:
...
<br> Influence of years since prior burn on FRP, all classes:
...
# Drop factors for which there are fewer than 10 points
  ...
  p <- ggplot(subset(x, year_int > 0), aes(year_int, log10(MaxFRP), group = year_int, fill = year_int)) + 
       geom_boxplot() + 
...
Influence of number of years since prior burn on FRP, by individual EVT class. Only significant (p < 0.05) results are shown.
...
# -------- PLOT by mean fire interval
  ...
  x.sub <- subset(x, evt_group == "Burned" & year_int != 0)
  x.summary <- x.sub %>% group_by(year_int) %>% summarize(mean = mean(MaxFRP))
...
#### **White Spruce**
Not significant but has a nutty outlier
```{r, echo=FALSE, warning=FALSE}
...
# -------- PLOT by mean fire interval
  ...
  x.summary <- x.sub %>% group_by(year_int) %>% summarize(mean = median(MaxFRP))
...
Histogram of FRP distributions
```{r, echo=FALSE}
