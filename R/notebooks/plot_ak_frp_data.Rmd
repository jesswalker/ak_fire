...
```
The above plot is not too helpful because FRP is evaluated across all land cover types.  Below is the same data broken out by land cover, where "land cover" is assigned to whichever EVT class covered the greatest proportion of each 1-km MODIS pixel.
```{r, echo=FALSE, eval=TRUE}
...
```
Black Spruce, Burned, Tundra, and Wetland appear to have possibly decreasing FRP relationships with burn number.  

White Spruce may have an increasing trend.

Look at each individually, with STATS.
