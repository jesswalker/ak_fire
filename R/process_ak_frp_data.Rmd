...
# Retain only specified columns
  ...
  "class31", 
  "class81", "class82",
  "MaxFRP", "latitude", "longitude", "sample",
...
# Set up df to hold entries for all points
  x.holder <- setNames(data.frame(matrix(ncol = 7, nrow = 0)),  c('ptid', 'lat', 'long', 'fire_year', 'frp', 'fireid', 'sample'))
...
create_new_fire_df = function(f) {
  ...
  merge1 <- left_join(f.new, distinct(dplyr::select(f, modis_year, frp, sample)), by = c("fire_year" = "modis_year"))
...
# Identify all rows in which fires past 2001 have an NA FRP
# dplyr rocks!
