...
# Retain only specified columns
  ...
  "class31", 
  "class81", "class82",
  "MaxFRP", "latitude", "longitude", "sample", "satellite",
  "newdate", "FireYear", "FIREID", "DiscDate")

 x.orig <- files.all[, colNames]
...
# Create 'classes' df with 1 entry for each ptid. Include the areal sum for each pixel
  x.classes <- x.orig %>% 
...
```
The ingested dataset has redundancies due to the fact that FRP pixels are paired with each coincident fire perimeter from all previous years. Example of a point with multiple associated fires (without class info):
...
# Identify all rows in which MODIS-era fires have an NA FRP
...
Fire year is assumed to be MODIS year for all fires 2002 - 2016. Other notes:
  ...
  * BLM fires after 2001 without a corresponding FRP are excluded; although the fire could've been missed due to clouds, it could also have been that the area didn't burn (the GIS database does not differentiate between burned and unburned areas )
...
# attempt to correct for the bowtie effect -- MEDIAN

temp.all <- x.fires.evt %>% group_by(sample) %>% summarize(frp_mean = mean(frp), frp_med = median(frp))

fit_med <- lm(frp_med ~ poly(sample, 2, raw=TRUE), data = temp.all)

coeff0 <- summary(fit_med)$coefficients[1, 1]
coeff1 <- summary(fit_med)$coefficients[2, 1]
coeff2 <- summary(fit_med)$coefficients[3, 1]

temp.all$fit_med <- coeff0 + (coeff1 * temp$sample) + (coeff2 * temp.all$sample^2)

ggplot(temp.all, aes(sample, frp_med)) + 
  geom_point() + 
  geom_line(data = temp.all, aes(sample, fit_med, color = 'red'), size = 1.2) +
  theme_bw()
```
 
```{r, echo = F,warning = FALSE}

#Make adjustments - MEDIAN
frp.min <- min(temp.all$fit_med, na.rm = T)

# Apply the minimum across the fit curve to calculate an adjustment ratio
temp.all$adj_med <- frp.min/temp.all$fit_med

# Multiply the actual frp by that adjustment 
temp.all$frp_med_new <- temp.all$frp_med * temp.all$adj_med

ggplot(temp.all, aes(sample, frp_med_new)) + geom_point() + theme_bw()
...
fit_mean <- lm(frp_mean ~ poly(sample, 2, raw = TRUE), data = temp.all)

coeff0 <- summary(fit_mean)$coefficients[1, 1]
coeff1 <- summary(fit_mean)$coefficients[2, 1]
coeff2 <- summary(fit_mean)$coefficients[3, 1]

temp.all$fit_mean <- coeff0 + (coeff1 * temp.all$sample) + (coeff2 * temp.all$sample^2)

ggplot(temp.all, aes(sample, frp_mean)) + 
  geom_point() + 
  geom_line(data = temp.all, aes(sample, fit_mean, color = 'Aqua'), size = 1.2) +
  theme_bw()
```
 
```{r, echo = F, warning=F,message=F}

#Make adjustments - MEAN
frp.min <- min(temp.all$fit_mean, na.rm = T)

# Apply the minimum across the fit curve to calculate an adjustment ratio
temp.all$adj_mean <- frp.min/temp.all$fit_mean

# Multiply the actual frp by that adjustment 
temp.all$frp_mean_new <- temp.all$frp_mean * temp.all$adj_mean

ggplot(temp.all, aes(sample, frp_mean_new)) + geom_point() + theme_bw()
...
# Calculate polynomial fit - MEDIAN - Terra

x.fires.evt.terra <- subset(x.fires.evt, satellite == '0')
temp.t <- x.fires.evt.terra %>% group_by(sample) %>% summarize(frp_mean = mean(frp), frp_med = median(frp))

fit_med <- lm(frp_med ~ poly(sample, 2, raw = TRUE), data = temp.t)

coeff0 <- summary(fit_med)$coefficients[1, 1]
coeff1 <- summary(fit_med)$coefficients[2, 1]
coeff2 <- summary(fit_med)$coefficients[3, 1]

temp.t$fit_med <- coeff0 + (coeff1 * temp.t$sample) + (coeff2 * temp.t$sample^2)

ggplot(temp.t, aes(sample, frp_med)) + 
  geom_point() + 
...
# Calculate polynomial fit - MEDIAN - Aqua

x.fires.evt.aqua <- subset(x.fires.evt, satellite == '1')
temp.a <- x.fires.evt.aqua %>% group_by(sample) %>% summarize(frp_mean = mean(frp), frp_med = median(frp))

fit_med <- lm(frp_med ~ poly(sample, 2, raw = TRUE), data = temp.a)

coeff0 <- summary(fit_med)$coefficients[1, 1]
coeff1 <- summary(fit_med)$coefficients[2, 1]
coeff2 <- summary(fit_med)$coefficients[3, 1]

temp.a$fit_med <- coeff0 + (coeff1 * temp.a$sample) + (coeff2 * temp.a$sample^2)

ggplot(temp.a, aes(sample, frp_med)) + 
  geom_point() + 
  geom_line(data = temp.a, aes(sample, fit_med, color = 'Terra'), size = 1.2) +
  theme_bw()
...
# Correct separately - TERRA

#Make adjustments - MEDIAN 
frp.min <- min(temp.t$fit_med, na.rm = T)

# Apply the minimum across the fit curve to calculate an adjustment ratio for each sample
temp.t$adj_med <- frp.min/temp.t$fit_med

# Now associate the sample # in the full database with the adjustment ratio
merge.t <- left_join(x.fires.evt.terra, distinct(dplyr::select(temp.t, sample, adj_med)), by = "sample")

# Multiply the actual frp by that adjustment 
merge.t$frp_adj <- merge.t$frp * merge.t$adj_med
...
test.t <- merge.t %>% group_by(sample) %>% summarize(frp_med = median(frp_adj))

ggplot(test.t, aes(sample, frp_med)) + geom_point(color = "blue") +
  geom_point(data=temp.t, aes(sample, frp_med))
