...
x.sub <- subset(x.fires.evt, !is.na(frp))
ggplot(x.sub, aes(frp)) + geom_histogram()
...
p <- ggplot(x.fires.evt, aes(as.factor(burn_num), frp)) +
  scale_y_log10() +
  geom_boxplot(aes(fill = as.factor(burn_num)), show.legend = F) +
  geom_point(data=means, aes(as.factor(burn_num), frp, shape = "mean"), size = 7) +
  scale_shape_manual("", values = 18) +
  geom_text(data = means, aes(label = round(frp, 1), y = frp + 0.7), 
            hjust = -0.1, vjust = -1.1) +
...
p <- ggplot(x.sub, aes(burn_num, frp)) + 
      geom_boxplot() + 
      scale_y_log10() +
...
p <- ggplot(x.sub, aes(x=frp, fill = burn_num)) + 
      scale_x_log10() +
      scale_y_log10() +
...
```
If proportion of max EVT class is at least 75%:
...
x.anova <- aov(formula = frp ~ evt_group * burn_num, data = x.fires.evt)
summary(x.anova)
...
x.anova <- aov(formula = frp ~ year_int * as.factor(evt_group) * burn_num, data = subset(x.fires.evt, reburn > 0))
summary(x.anova)
...
x.cut <- x.fires.evt[complete.cases(x.fires.evt[, 'frp']), ]
x.cut$bin <- cut(x.cut$frp, breaks = c(seq(0, 12000, 100)), include.lowest = T)

#x.test <- x.cut %>% group_by(bin) %>% count(.)
#x.test$frp <- seq(100, 5500, by = 100)
#x.test <- x.test %>% slice(1:(n()-1))


x.test <- x.cut %>% group_by(bin, fire_year) %>% count()
#x.test <- x.test[-(nrow(x.test)),]
x.test2 <- cbind(x.test, lower = as.numeric( sub("\\((.+),.*", "\\1", x.test$bin) ),
      upper = as.numeric( sub("[^,]*,([^]]*)\\]", "\\1", x.test$bin) ))
...
x.test3$fire_year <- as.factor(x.test3$fire_year)
